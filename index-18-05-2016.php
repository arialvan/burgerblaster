               ...
               <div class="row">
                   ...
                   <div class="col-md-3 col-sm-6 col-xs-6">
                    ...
                    <div class="col-md-3 col-sm-6 col-xs-6">
                        ...
                        $present=date('d')-1;
                        $today  =date('d');
                            $stmt12=$con->prepare("SELECT date_done, SUM(total) AS Masuk3 FROM order_done where id_outlet='$outlet' AND DAY(date_done)='$present' GROUP BY DAY(date_done) ASC");
                               $stmt12->execute();
                               $stmt12->setFetchMode(PDO::FETCH_ASSOC);	
                               $key2 = $stmt12->fetch();
                        ?>
                    ...
                    <div class="col-md-3 col-sm-6 col-xs-6">
                        <?php 
                            $stmt12=$con->prepare("SELECT date_done, SUM(total) AS Masuk FROM order_done where id_outlet='$outlet' AND DAY(date_done)='$today' GROUP BY DAY(date_done) ASC");
                               $stmt12->execute();
                               $stmt12->setFetchMode(PDO::FETCH_ASSOC);	
                               $key2 = $stmt12->fetch();
                        ?>
                        ...
                        <div class="text-box" >
                            <p class="text-temp">Penjualan hari ini <?php //echo DateToIndo($key2['date_done']);?></p>
                            <p class="text-muted"><?php echo rupiah($key2['Masuk']);?></p>
                        </div>
                        </div>
                    </div>   
