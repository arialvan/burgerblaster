...
case 'edit_material':
    $id_wh=$_POST['id'];
	  ...
	  $where = "id_material='".$fild."'"; 
         ...
         //Update tabel material_stock 
             $data2 = array( 'qty_material' => $_POST['qty_material']
				  );
				echo $db->update($tabel2,$data2,$where);
                                
				echo"Update Success";
				//$url="../material_edit.php?id_material=$_POST[id_material]&id=$id_wh";
...
case 'add_shareoutlet':
    ...
    // Insert tabel material price
    $fild="id_stock_outlet";
    $where="id_material='".$id_mat."' AND id_outlet='".$id_ot."'";
    $db->select($tabel4,$fild,NULL,$where);
    $row=$db->getResult();
    foreach ($row as $keys);
    $id_stock_outlet=$keys['id_stock_outlet'];
    
    $fild="*";
    $where="id_stock_outlet='".$id_stock_outlet."'";
    $db->select($tabel5,$fild,NULL,$where);
        if($hasil=($db->getResult())){
            echo'data ada';
        }else{
            $data = array( 'id_outlet'=> $id_ot,
				   'id_material' => $id_mat,
                                   'id_stock_outlet' => $id_stock_outlet,
                                   'price_sub' => 0
				  );
                                echo $db->insert($tabel5,$data);
        }
                                echo"Send Success";
				$url="../material_list.php?id=$_POST[id_wh]";
				echo "<meta http-equiv=\"REFRESH\" content=\"1;url=$url\">";
