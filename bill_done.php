...
$fild_order  = "product_code";
        ...
        foreach ($id_product as $key_pro => $values){   
            ...
            $wheres="$order_detail.id_pes='$fild' AND $product_detail.id_product='$id_pro' AND $product_detail.stock_qty!='0'";
...
//=========================    
                                ...
                                $fields_out=" *, ($material.price_sub*$x) AS Total";  
                                $wheres_out="$material.id_material='$key_detail[id_material]' AND $material.id_outlet='$id_ot' AND $outlet_stock.id_outlet='$id_ot' ";
                                ...
                                foreach($hasils_out as $key_out){
                                    ...
                                    //$ex= array_sum($total_mp);
                                    $material_purchase="material_purchase";
                                    $date=date('Y-m-d');
                                    $data = array( 'id_material' => $key_out['id_material'],
                                                   'id_outlet' => $id_ot,
                                                   'tgl_mp' => $_GET['tgl'],
                                                   'total_mp' => $total_mp
                                      );
